...
**5. 🏠 Прокачка дома (реальные данные, июль 2026).** Требований к прокачке **нет ни в публичном API, ни на официальной вики** — там только механика («нужны деньги и стройматериалы»). Цифры взяты с **Fletcher Wiki** (`fletcher-wiki.com`), снимком, как раньше гос-цены и рецепты:
   ...
   - **Материалы группируются по разделам**: проект получает позиции с `section` («Мастерская → ур. 3», «Кухня → ур. 5»), таблица рисует заголовок раздела с подытогом. Ключ позиции — `section|name` (`itemKey` в сторах), иначе одинаковый материал из разных разделов затирал бы сам себя.
